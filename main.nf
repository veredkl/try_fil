...
Channel
	...
	g_0_0_g_2 = Channel.empty()
 }

Channel.value(params.inputparam).set{g_3_1_g_2}
...
publishDir params.outdir, mode: 'copy', saveAs: {filename -> if (filename =~ /BC.*$/) "outputparam/$filename"}
input:
 set val(name),file(reads) from g_0_0_g_2
 val mate from g_3_1_g_2

output:
 set val(name),file("*_consensus-pass.fastq")  into g_2_reads00
 set val(name),file("BC*")  into g_2_logFile11
 set val(name),file("out*")  into g_2_logFile22
